...
if len(sys.argv) >= 3:
    ...
    print(
        'Script can be run from command line as <script> <model> <env prod or dev> <add attachment true/false> <date (optional)>')
    mode = 'DEVELOPMENT'
    add_attachment = False
    with open('/home/pi/Documents/trusted/option_email_list_dev.txt') as f:
        emaillist = f.read().splitlines()
...
PredictionVsStrikeIncrease(df, ReachedStrike, notReachedStrike, savefig=True,
                           saveFileName="scheduled_jobs/summary_content/scatter.png")
...
GroupsPerformanceComparisonBar(df, high_prob_df, high_prof_df, savefig=True,
                               saveFileName="scheduled_jobs/summary_content/strikePerBins.png")

# Nr of days until options reach their strike price
plotBarChartPlotly(df_days2strike, xcol='duration', ycol='strikeReachedShare',
                   titles={'title': 'Nr of days to reach strike price', 'xlabel': 'Days since extraction',
                           'ylabel': 'Share of active options'}, savefig=True,
                   saveFileName='scheduled_jobs/summary_content/daysToReachStrike.png')

##### Profitability
# Plot on expected profit when selling on strike (if reached)
ExpvsActualProfitabilityScatter(df, high_prob_df, high_prof_df, actualCol='profitPerc', savefig=True,
                                saveFileName="scheduled_jobs/summary_content/scatter_profitability.png")
# Plot on maximum profit if sold at max reached price
ExpvsActualProfitabilityScatter(df, high_prob_df, high_prof_df, actualCol='maxProfitability', savefig=True,
                                saveFileName="scheduled_jobs/summary_content/scatter_maxProfitability.png")
...
sendRichEmail(sender='k.sends.python@gmail.com'
              ...
              , inline_images=['scheduled_jobs/summary_content/scatter.png',
                               'scheduled_jobs/summary_content/CalibCurve.png',
                               'scheduled_jobs/summary_content/strikePerBins.png',
                               "scheduled_jobs/summary_content/daysToReachStrike.png",
                               'scheduled_jobs/summary_content/pr-threshold.png',
                               'scheduled_jobs/summary_content/roc.png',
                               'scheduled_jobs/summary_content/scatter_profitability.png',
